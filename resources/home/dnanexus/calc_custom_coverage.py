...
for file in os.listdir(folder):
    ...
    for depth in depths:  # list of integers
        ...
        basecount_above_depth = sum(
            hs_data.loc[hs_data["coverage_or_base_quality"] >= depth][
    ...
    hs_data_on_target_calc = pd.read_csv(
        hs_metrics_file, sep="\t", header=5, nrows=1
    )
    Usable_unique_bases_on_target = (
        hs_data_on_target_calc.ON_TARGET_BASES
        / hs_data_on_target_calc.PF_UQ_BASES_ALIGNED
    ) * 100
    sample_info["Usable unique bases on-target"] = float(
        Usable_unique_bases_on_target
    )
...
print(
    "Percentage coverage values were calculated for {} depths and saved to file: inputs/custom_coverage.csv".format(
        custom_depths
    )
)
